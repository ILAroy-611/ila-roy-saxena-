...
def find_flights(flight_time):
    ...
    for i in flight_details:
        word=i.split(":")
        if int(word[3])>=flight_time and int(word[3])<=flight_time+201:
            list1.append(i)
...
def sort_flight_list(flight_list):
    ...
    for i in range(0,len(list1)):
        flight_list.sort(key=lambda x:int(list1[i][3]))
    return flight_list
...
def get_passenger_details(flight_detail):
    ...
    for i in range(0,len(list1)):
        for key,value in passenger_details_dict.items():
            if list1[i][0]==passenger_details_dict[key][1]:
                result.append(key)
    return result
...
def security_check(passenger_pnr_list):
    ...
    for i in passenger_pnr_list:
        for key,value in passenger_details_dict.items():
            if i==key:
               if passenger_details_dict[key][3] >=0  and passenger_details_dict[key][3]<=25:
                   result.append(i)
    return result

def sort_passengers(passenger_pnr_list):
    updated_passenger_pnr_list=[]
    #Remove pass and write your logic here
    for i in passenger_pnr_list:
        new_dict={i:[]}
    for i in passenger_pnr_list:
        for key,value in passenger_details_dict.items():
            if i==key:
                new_dict[i]=passenger_details_dict[key][2]
    new_dict= sorted(new_dict.items())
    for i in new_dict:
        updated_passenger_pnr_list.append(i[0])
    return updated_passenger_pnr_list
    
def boarding(passenger_pnr_list): 
    boarding_queue=Queue(len(passenger_pnr_list))
    #Remove pass and write your logic here
    for i in passenger_pnr_list:
        boarding_queue.enqueue(i)
