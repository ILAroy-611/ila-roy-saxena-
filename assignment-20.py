...
class Item:
    ...
    def __str__(self):
        return "name of the item is "+str(self.__item_name)+" name of the author is "+str(self.__author_name)+" published in the year "+str(self.__published_year)
...
class Library:
    ...
    def sort_item_list_by_author(self,new_item_list):
        new_item_list.sort(key=lambda x:x.get_author_name().replace(" ",""))
        return new_item_list
    
    def add_new_items(self,new_item_list):
        list1=self.__item_list+new_item_list
        self.__item_list=self.sort_item_list_by_author(list1)
        
    def sort_items_by_published_year(self):
        self.__item_list.sort(key=lambda x:(x.get_published_year(),x.get_author_name().replace(" ","")))
